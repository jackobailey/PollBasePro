...
The PollBase data come from a wide range of sources. This includes books published after each general election, polling almanacs, data shared amongst academics and pollsters, contemporary media reports, and figures from polling company websites. The data since the 1983 general election are complete, baring possible but rare individual errors. Prior to 1983, the data is likewise complete for each general election campaign period. For periods between general elections prior to 1983, coverage is more comprehensive for some pollsters than others^[We encourage any readers who might know of polls missing from the PollBase dataset to contact its author via his website (https://www.markpack.org.uk)]. Gallup and NOP data, in particular, are well-covered as the two companies have collated and published their results. The PollBase data start in `r format(min(pollbase$start), "%B %Y")`. However, we take as our starting point the 1955 Parliament given the gaps in the earlier data.
