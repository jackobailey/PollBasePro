...
The `pollbasepro` data suggest that the `r txt_dta$party[1]` `r ifelse(txt_dta$party[1] %in% c("Conservative Party", "Labour Party"), "is", "are")` the largest party in Britain. They hold a lead over the `r txt_dta$party[2]` of around `r paste0(scales::percent(txt_dta$est[1] - txt_dta$est[2]), " (", scales::percent((txt_dta$est[1] - txt_dta$est[2]) - qnorm(.975)*sqrt(txt_dta$err[1]^2 + txt_dta$err[2]^2)), " to ", scales::percent((txt_dta$est[1] - txt_dta$est[2]) + qnorm(.975)*sqrt(txt_dta$err[1]^2 + txt_dta$err[2]^2)), ")")`. This puts the `r txt_dta$party[2]` in second and the `r txt_dta$party[3]` in third.
